...
grid = [
    ...
    [04, 52, 8, 83, 97, 35, 99, 16, 07, 97, 57,
...
def euler_011(data=grid, num_consecutive=4):
    greatest_product = 1

    for i in xrange(grid_size):
        for j in xrange(grid_size):
            # reset each direction product
            curr_products = [greatest_product, 1, 1, 1, 1]
            for k in xrange(num_consecutive):
                try:
                    # check horizontally (ltr)
                    curr_products[1] *= data[i][j + k]
                    # check vertically (top-down)
                    curr_products[2] *= data[i + k][j]
                    # check diagonally (top-down,ltr)
                    curr_products[3] *= data[i + k][j + k]
                    # check diagonally (down-up,ltr)
                    curr_products[4] *= data[i + k][19 - j - k]
                except IndexError:
                    pass
            greatest_product = max(curr_products)
