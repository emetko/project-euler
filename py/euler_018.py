...
# start from the triangle base
# walking up, substitute each value with the max sum possible
# max sum for the whole triangle will be at 0,0


def euler_018(triang=triangle_string):
    triang = [x.split(' ') for x in triang[1:-1].split('\n')]
    for i in reversed(xrange(len(triang) - 1)):
        for j in xrange(i + 1):
            triang[i][j] = int(
                triang[i][j]) + max(int(triang[i + 1][j]), int(triang[i + 1][j + 1]))
    return triang[0][0]
